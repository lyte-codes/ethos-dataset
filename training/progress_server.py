...
def collect(output: Path, log: Path, pattern: str) -> dict:
...
def make_handler(output: Path, log: Path, pattern: str, page: Path):
    class Handler(BaseHTTPRequestHandler):
        def do_GET(self):  # noqa: N802 — BaseHTTPRequestHandler's interface
            if self.path.startswith("/data.json"):
                body = json.dumps(collect(output, log, pattern)).encode()
...
def main() -> int:
    ...
    server = HTTPServer(("127.0.0.1", args.port), make_handler(args.output, args.log, args.pattern, args.page))
